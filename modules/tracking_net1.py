...
from .fuse_proj import fuse_extract
import cv2
import numpy as np
...
class TrackingNet(nn.Module):
    ...
    def __init__(self,
        ...
        if point_len == 0:
            ...
            self.stn_layers, self.point_net_layers, self.point_fp_layers = parse_pointnet_layers(
                point_in_channels,
    ...
    def feature(self, dets, det_info):
        ...
        appearance_pool1 = self.appearance_pool[0]
        appearance_pool2 = self.appearance_pool[1]
        appearance_pool3 = self.appearance_pool[2]
        appearance_pool4 = self.appearance_pool[3]
        ...
        pointnet_fp4 = self.point_fp_layers[0]
        pointnet_fp3 = self.point_fp_layers[1]
        pointnet_fp2 = self.point_fp_layers[2]
        pointnet_fp1 = self.point_fp_layers[3]
        ...
        points_xy1, points_xyz1, points1, points_split1 = pointnet_sa1(points_xy0, points_xyz0, points0, points_split0)
        #appear1, points1 = self.fuse_net(points_xy1, appear1, points1, points_split1, M_matrixes1)
        ...
        points_xy2, points_xyz2, points2, points_split2 = pointnet_sa2(points_xy1, points_xyz1, points1, points_split1)
        #appear2, points2 = self.fuse_net(points_xy2, appear2, points2, points_split2, M_matrixes2)
        ...
        points_xy3, points_xyz3, points3, points_split3 = pointnet_sa3(points_xy2, points_xyz2, points2, points_split2)
        #appear3, points3 = self.fuse_net(points_xy3, appear3, points3, points_split3, M_matrixes3)
        appear_pool_out.append(appear3)

        # 4th layer
        M_matrixes4 = torch.cat([torch.div(M_matrixes3[:, :2, :],2), M_matrixes3[:,2:, :]],dim = 1)
        appear4 = appearance_conv4(appear3)
        #appear4 = appearance_pool4(appear4)
        points_xy4, points_xyz4, points4, points_split4 = pointnet_sa4(points_xy3, points_xyz3, points3, points_split3)
        #appear4, points4 = self.fuse_net(points_xy4, appear4, points4, points_split4, M_matrixes4)

        appear_pool_out.append(appear4)
        
        points3 = pointnet_fp4(points_xyz3, points3, points_split3,points_xyz4, points4, points_split4)
        points2 = pointnet_fp3(points_xyz2, points2, points_split2,points_xyz3, points3, points_split3) 
        points1 = pointnet_fp2(points_xyz1, points1, points_split1,points_xyz2, points2, points_split2) 
        points0 = pointnet_fp3(points_xyz0, None, points_split0,points_xyz1, points1, points_split1) 
        ...
        points0 = point_conv1(points0)
        points_out = []
        for i in range(len(points_split0) - 1):
            start = points_split0[i].item()
            end = points_split0[i + 1].item()
            points_pool = avg_pool(points0[:, :, start:end])
