...
class fusion_module_D(nn.Module):

    def __init__(self, appear_len, point_len, out_channels):
        super(fusion_module_D, self).__init__()
        print(
            "Fusion Module D: split sigmoid weight gated point, image fusion")
        self.appear_len = appear_len
        self.point_len = point_len
        self.extract = nn.Sequential(
            nn.Conv1d(appear_len + point_len, 128, 1, 1),
            nn.Conv1d(128, 256, 1, 1),
            nn.Conv1d(256, out_channels, 1, 1),
            nn.GroupNorm(out_channels, out_channels)
        )
    ...
    def forward(self, objs):
        """
            objs : 1x2DxL
        """
        feats = objs.view(1, -1, objs.size(-1))  # 1x2DxL -> 2xDxL
        obj_feats = self.extract(feats)
        return obj_feats
